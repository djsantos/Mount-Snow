  ...
  for (var i = 0; i < len; i++) {
   ...
   var unique = db.run("SELECT COUNT(*) FROM UserTable;");
            ...
            function (callback){
              ...
              var uid = db.get("select * from UserTable where uid=(select MAX(uid) from UserTable)");/*, function (error){
                if (error){
                  cb(error);
                }
                callback(null);
            ...
            /*function(callback){
              db.get("select * from UserTable where uid=(select MAX(uid) from UserTable)", function(error, row){
                if (error){
                  cb(error);
                }
                callback(null, row);
