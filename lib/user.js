...
exports.lookup = function(username, password, cb) {
  var len = userdb.length;
   db.run("CREATE TABLE IF NOT EXISTS tweetTable (message VARCHAR(140) not null, uid INTEGER not null, username VARCHAR(30) not null);");
...
exports.createUser = function(name,email ,password, confirmPassword, username, cb){
  ...
  for (var i = 0; i < len; i++) {
   ...
   var unique = db.run("SELECT COUNT(*) FROM UserTable;");
                ...
                db.run('insert into UserTable values (1, slastic, Santos,djsantos@student.umass.edu);');
                db.run('insert into UserTable values (2, dante, password,ddebened@student.umass.edu);');
                db.run('insert into UserTable values (3, maxwell24, Bohling,mbohling@student.umass.edu);');
                db.run('insert into UserTable values (4, bmentz, nimda,bmentzer@student.umass.edu);');
                    async.series([
            function (callback){
