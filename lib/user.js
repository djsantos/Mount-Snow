...
exports.lookup = function(username, password, cb) {
  ...
  var record = db.all("SELECT COUNT(*) FROM UserTable where username = ?;", username);
  //var record = db.get("SELECT uid FROM UserTable where username = ?;", username);
  console.log(record);
  record = parseInt(record);
  if(record > 0){
  	 var user = db.run("select * from UserTable where username = ?", username);
  	cb(undefined, user);
  }
  else{
  	cb('user not found');
