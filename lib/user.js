...
exports.lookup = function(username, password, cb) {
  ...
  db.run("SELECT EXISTS(SELECT 1 FROM users WHERE username=? LIMIT 1);", username);
  for (var i = 0; i < len; i++) {
    var u = userdb[i];
...
exports.createUser = function(name,email ,password, confirmPassword, username, cb){
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (u.username === username) {
      ...
      cb(undefined, uid);
      var unique = db.run("SELECT COUNT(*) FROM users;");
      unique = parseInt(unique);
      	db.run("CREATE TABLE IF NOT EXISTS users (uid INTEGER PRIMARY KEY autoincrement,username VARCHAR(30) not null,password VARCHAR(30) not null,email VARCHAR(30) not null);");
    db.run("INSERT into users values (?, ?, ?, ?)", [unique+1, username, password, email]);
};
