...
var connect = [
  new Connection("Hi Dan", 1, 4),
  new Connection("Hi Dante", 2, 1),
  new Connection("Hi Max", 3, 2),
  new Connection("Hi Bridgette", 4, 3),
];

//database for tweets that mention a user
var mention = [
  new Connection("Hi Dan @slastic", 1, 4),
  new Connection("Hi Dante @dante", 2, 1),
  new Connection("Hi Max @maxwell24", 3, 2),
  new Connection("Hi Bridgette @bmentz", 4, 3)
];
...
exports.displayMention = function(id){
  ...
  if(!mention)
  	...
  	for(var x = 0; x < mention.length; x++) {
		if(mention[x].uid === id){
