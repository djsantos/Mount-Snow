...
var messages = [ 
new DirectMessage('Hi, my name is daniel', 1, 2),// message, from, to
new DirectMessage('Hi, my name is dante', 2, 1),// message, from, to
];
	...
	var len = messages.length;
  	for (var i = len-1; i >= 0; i--) {
		if(id === messages[i].uid2){
    		result.push(user.getUsername(messages[i].uid));
	...
	var len = messages.length;
  	for (var i = len-1; i >= 0; i--) {
		if(id === messages[i].uid){
			result.push(user.getUsername(messages[i].uid2));
    		result.push(messages[i].message);
    	}
    }
    return result;
...
//adds messgae to the messages database
exports.createMessage = function(message, id, id2, cb){
	console.log(id2);
	if(!message)
		cb('no message');
