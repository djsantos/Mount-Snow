...
this.uid = uid;
this.username = user.getUsername(uid);
}
...
exports.createTweet = function(message, id, cb){
	...
	if(name.length > 140){
		...
		var tags = name.match(/@/g);
		//you can tag up to 2 people
		if(tags.length === 2){
				name = name.substring(name.indexOf("@", name.indexOf("@", 0)), name.indexOf(" ", name.indexOf("@", name.indexOf("@", 0))))
				id = user.getUid(name);
				console.log(id);
