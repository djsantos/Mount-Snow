...
exports.displayTweets = function(id, cb){
	...
	//nested query
	var temp = db.get("select * tweetTable where uid = ?", [id]);
...
exports.tweetCount = function(id){
	...
	db.run("SELECT COUNT(*) FROM tweetTable where uid = ?;", [id], function(err,num){
  		console.log(num);
  		if(num){
  			console.log(num);
  			cb(undefined, num);
	...
	var done = false;
		...
		/*old else
            ...
            db.run("CREATE TABLE IF NOT EXISTS tweetTable (message VARCHAR(140) not null, uid INTEGER not null, username VARCHAR(30) not null);");
                    async.series([
            function (callback){
              db.run("insert into tweetTable values (?, ?, ?);", [message, uid, username]);
              cb(undefined, uid);
            }]);
                }
	}
